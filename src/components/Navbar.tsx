  ...
  return <nav className={cn('fixed top-0 left-0 w-full z-50 transition-all duration-300 py-4', scrolled ? 'bg-white/90 backdrop-blur-md shadow-md' : 'bg-transparent')}>
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <button className="md:hidden text-yoga-navy hover:text-yoga-terracotta transition-colors" onClick={toggleMenu}>
      ...
      {/* Mobile menu */}
      <div className={cn('fixed top-[76px] left-0 w-full bg-white shadow-lg md:hidden transition-transform duration-300', isOpen ? 'translate-y-0' : '-translate-y-full')}>
